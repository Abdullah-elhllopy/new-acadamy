...
export const trainerSchema = z.object({
  ...
  instructorNameAr: z.string().min(2, 'Trainer name (Arabic) is required'),
  instructorBio: z.string().min(10, 'Bio must be at least 10 characters'),
  instructorBioAr: z.string().min(10, 'Bio (Arabic) must be at least 10 characters'),
  specialization: z.string().min(2, 'Job title is required'),
  specializationAr: z.string().min(2, 'Job title (Arabic) is required'),
  instructorEmail: z.string().email('Invalid email address'),
  instructorPhone: z.string().min(10, 'Phone number must be at least 10 digits'),
  videoUrl: z.string().url('Invalid video URL').optional().or(z.literal('')),
  experience: z.coerce.number().min(0, 'Experience must be at least 0').max(50, 'Experience must be less than 50'),
