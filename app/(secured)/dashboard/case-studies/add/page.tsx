...
import { useCreateCaseStudy } from '@/hooks/api/use-case-studies'
...
export default function AddCaseStudyPage() {
  ...
  const onSubmit = (data: z.infer<typeof caseStudySchema>) => {
    ...
    createCaseStudy.mutate(formData, {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-primary">
          {isArabic ? 'إضافة دراسة حالة جديدة' : 'Add New Case Study'}
        </h1>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <FormField
                ...
                render={({ field: { value, onChange, ...field } }) => (
                  <FormItem>
                    <FormLabel>{isArabic ? 'شعار العميل' : 'Client Logo'}</FormLabel>
                    <FormControl>
                      <Input
                        type="file"
                        accept="image/*"
          ...
          <div className="flex gap-4">
            <Button type="submit" disabled={createCaseStudy.isPending}>
              {createCaseStudy.isPending ? (isArabic ? 'جاري الحفظ...' : 'Saving...') : (isArabic ? 'حفظ' : 'Save')}
            </Button>
            <Link href="/dashboard/case-studies">
